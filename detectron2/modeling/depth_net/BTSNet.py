...
#
# This file is a part of Supervised.
...
from detectron2.modeling.depth_net import DEPTH_NET_REGISTRY
...
class atrous_conv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation, apply_bn_first=True):
        ...
        if apply_bn_first:
            self.atrous_conv.add_module('first_bn',
                                        nn.BatchNorm2d(in_channels, momentum=0.01, affine=True,
...
class bts(nn.Module):
    def __init__(self, dataset, max_depth, feat_out_channels, num_features=512):
...
class encoder(nn.Module):
    def __init__(self, encoder_name):
        ...
        logger.info(f'Bypass checkpointor, '
                    f'directly load {encoder_name} from torchvision.models')
...
class BtsModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.encoder_name = cfg.MODEL.DEPTH_NET.ENCODER_NAME
        self.encoder = encoder(cfg.MODEL.DEPTH_NET.ENCODER_NAME)
        self.decoder = bts(cfg.MODEL.DATASET, cfg.MODEL.MAX_DEPTH,
...
def set_misc(model, bn_no_track_stats, fix_first_conv_block, fix_first_conv_blocks):
    if bn_no_track_stats:
        logger.info("Disabling tracking running stats in batch norm layers")
        model.apply(bn_init_as_tf)

    if fix_first_conv_blocks:
        if 'resne' in model.encoder_name:
...
if __name__ == '__main__':
    ...
    params = namedtuple('params', 'encoder max_depth dataset bts_size')
    p = params(encoder='resnet50_bts', max_depth=100, dataset='kitti', bts_size=512)
    ...
    y = net(torch.randn(2, 3, 512, 512).cuda(), torch.tensor([1.0]).cuda())
    print(y[0].size())
