...
class Camera(nn.Module):
    ...
    def __init__(self, K, Tcw=None):
        ...
        super().__init__()
        self.K = K
        self.Tcw = torch.eye(4, dtype=torch.float).repeat([len(K), 1, 1]) if Tcw is None else Tcw
    ...
    def reconstruct(self, depth, frame='w'):
        ...
        # If in world frame of reference
            ...
            B, _, H, W = Xc.shape
            out = self.Twc[:, :3, :3].bmm(Xc.view(B, 3, -1)) + self.Twc[:, :3, -1].unsqueeze(-1)
    ...
    def project(self, X, frame='w'):
        ...
        if frame == 'c':
            ...
            B, _, H, W = X.shape
            out = self.Tcw[:, :3, :3].bmm(X.view(B, 3, -1)) + self.Tcw[:, :3, -1].unsqueeze(-1)
            Xc = self.K.bmm(out)
        else:
            raise ValueError('Unknown reference frame {}'.format(frame))
